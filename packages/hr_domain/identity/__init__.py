"""服务端可信身份映射 — 共享 Authority。

数据来自服务端 EMPLOYEE_IDENTITY_MAP_JSON，把内部伪匿名 user_id 解析为真实
employee_id，并生成不可逆的 employee_ref。此模块是 Leave 与 Employee Data
两个 Agent 共同依赖的唯一身份解析 Authority；任何 Agent 不得再自建一套平行
resolver。A2A user_id 绝不直接作为 employeeId。
"""
