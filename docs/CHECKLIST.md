...
组织回答）都付一次完整 thinking 开销，单跳 400~1800 字。聊天场景下 27s 中位
延迟基本不可接受，上线前需要在以下方向取舍（**质量 vs 延迟，需业务拍板**）：

1. **调 thinking 档位**——方舟支持 thinking 开关/档位。关掉延迟降幅最大，但
   多步校验链（性别/排班/余额顺序判断）与知识库结果筛选依赖推理，需重跑评测
   确认质量不掉（当前 22 条断言可直接用作回归基准）
2. **减少 transfer 跳数**——把高频意图（余额、医疗期、页面跳转、固定话术）留在
   root_agent 直接处理。实测这类已是 4~14s；反之凡走 transfer 的都 30s+
3. **换更快的模型**——如 doubao-seed-1.6-flash，同样需重跑评测对比质量
4. **前端体感优化**——流式输出 + "正在查询排班…"的过程提示，把等待可视化
