...
def stub_backend(monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.consult_agent.tools.kb_search.get_backend", lambda: LocalStubBackend()
    )
...
def test_kb_search_backend_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.consult_agent.tools.kb_search.get_backend",
...
def test_kb_search_exposes_partial_failure_without_changing_data_shape(monkeypatch):
    from apps.consult_agent.knowledge.backend import KnowledgeSearchResults
